...
const COMMANDS := {
	...
	"bugreport": "dump seed/pos/screenshot/session log to a zip in user://bugs",
...
func _prof_refresh() -> void:
	...
	# AC-0262: the LOW sub-part breakdown (LOW_POLL) joins the live table —
...
func dispatch(s: String) -> void:
	...
	match cmd:
		...
		"bugreport":
			Debug.bug_report()
			add_log("bugreport: capture started (zip in user://bugs)")
		"quit":
			add_log("[quitting...]")
			get_tree().quit()
			return
