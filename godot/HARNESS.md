...
| `r16` | `_r16_test` (AC-0212) | 16-radius build + moving inside a chunk: per-frame CPU in static vs moving phases (frame ms → effective fps), camera on a 12 m circle inside the spawn chunk (frustum changes every frame) | `ok`, `radius` (16), `ho_cap` (AC-0225: the "chunks_per_frame" handoff cap in force), `built`/`built_all`/`total_chunks` (1089), `resident`, `build_ms`, `cull_mode` (`engine` default / `manual`), `perf_cull_passes`/`perf_cull_flips` (manual-pass counters for the WHOLE arm — 0/0 in engine mode), `static`/`moving` {`n`,`p50_ms`,`p95_ms`,`max_ms`,`fps_p50`,`fps_p95`,`fps_min`}, `drop_fps_static_vs_moving` | `AWECRAFT_FRUSTUM` (engine [default] / manual / 0), `AWECRAFT_RADIUS` (forced ≥16 by the arm) | LONG — run `--fixed-fps 600` so ticks are uncapped and the frame-ms awaits measure true per-frame CPU (same convention as breakspike); ~3–8 min wall (r16 build, 25-min in-arm cap, proceeds on partial build) | AC-0212 #1 gate: A/B `AWECRAFT_FRUSTUM=manual` vs default engine — no 5 fps drop in the moving phase (the removed manual pass re-evaluated every resident chunk on every camera change). AC-0313: the arm's sim pin moved 2 → 4 (the new SIM_MIN floor; 2 is no longer storable), medium_start 4 → 5 — the real band rebases [0,2] → [0,4] and the build/settle + section counts rebase (documented delta; the coordinator re-measures, the assertions read band0_r live) |
...
| `halo` | `_halo_test` (AC-0283 P3, AC-0286 promo block) | Settled-world gate for the heightmap-sky far band + promotion/demotion lifecycle, render 6 (real taxi ≤ 4 + halo taxi 5; taxi ≥ 6 data-only): (a) every real column seeded+settled ≡ the legacy reference; (b) every halo column UNseeded and every applied 4×4 low mesh ≡ a fresh h_avg_emit on the arm-recomputed heightmap sky (15 strictly above the terrain top); (c) promotion of a known halo column — settled ≡ reference, single flood (seed-at-landing == 1), one enqueue per residency, retain-swap (0 promotion-originated hide late-landings); (d) demotion re-lowers the low | `a` {`seeded`, `settled`, `mismatches`, `boundary` (26 = the deep-corner under-lit-until-promotion class)}, `b` {192/192 byte-identical}, `c` {`seed` (1), `enq` (1), `land`, `late_landings_promo` (0)}, `d`; `ok` | `AWECRAFT_LOGIC=halo` | ~3–5 min | (a)'s 26 boundary mismatches are the documented under-lit-until-promotion class (the legacy reference sees far columns as holes; the arm fills them with the skip rule). The (c) promo block is the AC-0286 evidence (single flood + one promotion per residency + no hide) |
| `lightstate` | `_lightstate_test` (AC-0273) | Per-slab light-state canaries on an R16 moving camera: the slab's opaque-surface vertex colors keyed by quantized position (baked-light A/B — first landing vs edit rebuild), the double-LOD canary and the sky-wash census | `ok`, per-chunk `leaks`/`air_n`/`mean_eff`, `worst` {`chunk`,`cell`,`before`,`after`}, `double_lod` (0/0), `wash` (false), census deltas (documented timing) | `AWECRAFT_LOGIC=lightstate` | LONG — r16 class (~10–20 min, `--fixed-fps 600` convention) | The R16 canaries are EXACT across rebases (census deltas = documented timing, not drift): the AC-0283..0286 series keeps `double_lod` 0/0 and `wash` false |
| `ladder` | `_ladder_test` (AC-0261, rebuilt at AC-0283 P3, rebased at AC-0313) | Halo average-color ladder: small world (render 8, sim 4 — the AC-0313 SIM_MIN floor; the real band taxi ≤ band0_r, taxi metric) — REAL [0, band0_r] = per-slab full meshes, the HALO band = 4×4×4 placeholder slabs; asserts the per-tier average colors are byte-identical across the emit paths + the boundary flip + the air checks + the (e) Y-layer-rank encoding (the intra-column (taxi, layer) order) | `ok`, `ab_ok` (avg-color A/B), `boundary_flip_ok`, `air_synth_ok`, `air_inworld_ok`, `layerrank_ok` (AC-0313: the tier-0 SET test `tier0_ok` is gone with the tier-0 set), `clutter_ok`, slab counts (**REBASED at AC-0313 (measured 2026-09-17): `band_slabs`/`low_slabs` 695, `sky_mixed_slabs` 44, `med_slabs` 0, `sky_bad_alpha` 0** — the real band grew from taxi ≤ 2 (13 cols) to taxi ≤ 4 (41 cols) so the HALO band shrank; `med_slabs` 0 records that the 8×8 med tier is still NOT a draw tier — AC-0312 restores it) | `AWECRAFT_LOGIC=ladder` | ~2–4 min | the AC-0261 med/low ladder on the AC-0283 P3 halo band; AC-0313 rebased the slab counts (966/57 → 695/44, documented delta — the byte-identity across emit paths is the invariant, not the numbers) |
...
| `farab` | `_farab_test` (AC-0284b) | FAR (h-only) column A/B battery: (a) skip-filled slabs (`generate_resl` skip=1 — the 284a shape, kept as the reference) through low_emit_avg vs (b) the far payload through `AweMesh.h_avg_emit` — identical grid inputs + fcc/sky must give byte-identical output; plus the v6 codec round-trip | `ok`, `cols` (984), `h_mismatch` (0), `slab_cmp` (984), `slab_mismatch` (0), `codec` {payload size ~198 B, round-trip exact}, `fcc_tiles` | `AWECRAFT_LOGIC=farab` | ~2–4 min | The AC-0284b invariant: the far H-only payload (H u16 + biome + top, v6 flag bit 1) emits byte-identical meshes — H is the heightmap and stays bit-exact with the full path so promotion never shifts terrain |
