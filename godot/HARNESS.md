...
| `ladder` | `_ladder_test` (AC-0261, rebuilt at AC-0283 P3) | Halo average-color ladder: small world (render 8, sim 2 — the real band taxi ≤ band0_r, taxi metric) — REAL [0, band0_r] = per-slab full meshes, the HALO band = 4×4×4 placeholder slabs; asserts the per-tier average colors are byte-identical across the emit paths + the boundary flip + the air checks | `ok`, `ab_ok` (avg-color A/B), `boundary_flip_ok`, `air_synth_ok`, `air_inworld_ok`, `tier0_ok`, `clutter_ok`, slab counts (966, of which 57 sky-mixed) | `AWECRAFT_LOGIC=ladder` | ~2–4 min | 966==966 byte-identical across bands with only ms-timing deltas across rebases (the AC-0261 med/low ladder on the AC-0283 P3 halo band) |
...
| boundary r4 one-shot | `ok`/`remesh_ok`/`marker_ok` true, walk `p95_ms` **≈ 22** (p50 ~14) — **re-measured 2026-09-17 at the AC-0286 closeout (20 crossings, walk_s 44.8): p95 22, p50 14, `max_ms` 128 (was ~1.3–1.4 s — first-frame work rebuilt by AC-0283 P2/P3), `built_final` 43 (was ~50–52), `resident_final` 93 (was ~101), `queue_size` 81 (was ~54–56), `staged_dropped` 707, one 9.2 s burst at a far crossing (the AC-0178 non-fatal class was 17–40 s)** — **supersedes** ≈ 30–37 (AC-0178: p50 ~13, p95 30–37 — the group-consumption fix un-stuck the recenter 5×5 burst prune, front-loading work into smaller bursts and smoothing per-frame walk cost), **which superseded** ≈ 73–89 (AC-0155; re-verified 74/80 at AC-0158/0164), ≈ 54 (AC-0152/0160) and 267–401 / ~39.9k–47.6k VOID (banded streaming: circle ∪ diamond ∪ ring set, spawn-fast drain). **RESOLVED (AC-0178):** the engine-level `ERROR: Invalid Task ID` at `is_task_completed` from `recenter` (world.gd) is GONE — the 5×5 burst is a GROUP task; the recenter prune uses `is_group_task_completed` + `wait_for_group_task_completion`. Zero SCRIPT ERROR in all runs since | AC-0286 closeout (2026-09-17) supersedes AC-0178 (2026-09-01) supersedes AC-0155 (2026-08-31) supersedes AC-0152/0160 supersedes AC-0091/AC-0143 |
