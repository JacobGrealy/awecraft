...
| farab `slab_mismatch` | **0 / 984** (984/984 byte-identical) — the far H-only payload (H u16 + biome + top, ~198 B on disk, v6 flag bit 1) emits byte-identical meshes through `AweMesh.h_avg_emit` | AC-0284b (H stays bit-exact with the full path so promotion never shifts terrain) |
| halo (b) heightmap-sky re-emit | **192/192** byte-identical — every applied 4×4 low mesh ≡ a fresh h_avg_emit on the arm-recomputed heightmap sky (15 strictly above the terrain top over the quad footprint); the 26 boundary cells of (a) are the documented under-lit-until-promotion class, not a failure | AC-0284b (AC-0284a was 0 — the 26 = the legacy-reference far-hole class) |
