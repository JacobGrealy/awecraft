...
### Known-stable gate values (FRESH runs — RE-ESTABLISHED 2026-08-29 by AC-0091, H 80→384; AMENDED 2026-08-31 by AC-0152/0160 banded streaming)
...
| boundary r4 one-shot | `ok`/`remesh_ok`/`marker_ok` true, walk `p95_ms` **≈ 54** (AC-0152/0160 one-shots: builder 53, coordinator 54), `p50` 6, `max_ms` ~1076, `built_final` ~53, `resident_final` ~101, `queue_size` ~56 — **NEW band; old 267–401 / ~39.9k–47.6k VOID** (banded streaming: circle ∪ diamond ∪ ring set, spawn-fast drain; the walk path also healed by the never-sync drain fix). **Known non-fatal anomaly (follow-up recommended):** one 20–26 s burst at crossing 16 (`burst_max_ms` 21132/24916/26438 across three runs) + engine-level `ERROR: Invalid Task ID` at `is_task_completed` from `recenter` (world.gd:2258) — present in every run of the post-AC-0152 streaming code, non-blocking (run completes, queue drains) | AC-0152/0160 (2026-08-31) supersedes AC-0091/AC-0143 |
