...
| boundary r4 one-shot | `ok`/`remesh_ok`/`marker_ok` true, walk `p95_ms` **≈ 73–89** (AC-0155 one-shots: 73/89 — small upward drift from the write-on-evict save-queue, 1–2 cols/frame ~26 ms encode, predicted at handoff), `p50` 0–1, `max_ms` ~1.3–1.4 s, `built_final` ~50–52, `resident_final` ~101, `queue_size` ~54–56 — **band updated; old ≈ 54 (AC-0152/0160) and 267–401 / ~39.9k–47.6k VOID** (banded streaming: circle ∪ diamond ∪ ring set, spawn-fast drain; the walk path also healed by the never-sync drain fix). **Known non-fatal anomaly (follow-up recommended):** one burst at crossing 16 (`burst_max_ms` 37762/38671 at AC-0155; 21132/24916/26438 at AC-0152/0160) + engine-level `ERROR: Invalid Task ID` at `is_task_completed` from `recenter` (world.gd) — present in every run of the post-AC-0152 streaming code, non-blocking (run completes, queue drains). **Re-verified at AC-0158+AC-0164: one-shots 74/80 in-band** [20 Hz tick + threaded column I/O — the write-on-evict main-thread cost is now on a worker, band holds; crossing-16 burst 33.3/40.1 s, same class] | AC-0155 (2026-08-31) supersedes AC-0152/0160 supersedes AC-0091/AC-0143 |
